...
class LoginActivity : AppCompatActivity() {
                ...
                auth.signInWithEmailAndPassword(email,password).addOnCompleteListener {task->
                    if (task.isSuccessful){
                        if (auth.currentUser!!.isEmailVerified){
                            ...
                            showProgressBar(false)
                            auth.signOut()
                            Snackbar.make(binding.root, "Please verify Your email \n(check your spam email if you can't find our email)", Snackbar.LENGTH_LONG).show()
                        }
                    }else{
                        showProgressBar(false)
    ...
    private fun checkUserState(){
        if (auth.currentUser !=null){
            ...
            //finish()
            Log.d("LoginActivity","${auth.currentUser!!.isEmailVerified} ,, ${auth.currentUser!!.displayName}"  )
        }
    }
